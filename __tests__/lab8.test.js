describe("Basic user flow for Website", () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    let innerTextProp = await shadowbutton.getProperty("innerText");
  ...
  it("Checking number of items in cart on screen", async () => {
    ...
    for (let i = 1; i < prodItems.length; i++) {
      ...
      let shadowbutton = await shadowroot.$("button");
      console.log(`Simulating Click on Item ${i + 1}!`);
      await shadowbutton.click();

      let cart = await page.$("#cart-count");
      let cartcount = await cart.getProperty("innerText");
      console.log("Items in Cart: " + cartcount["_remoteObject"].value);
      expect(cartcount["_remoteObject"].value).toBe(`${i + 1}`);
    }
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let shadowroot = await prodItems[i].getProperty("shadowRoot");
      let shadowbutton = await shadowroot.$("button");
      console.log(`Checking 'Remove from Cart' for item ${i + 1}`);
      let innerTextProp = await shadowbutton.getProperty("innerText");
      expect(innerTextProp["_remoteObject"].value).toBe("Remove from Cart");
    }
    ...
    let cart = await page.$("#cart-count");
    let cartcount = await cart.getProperty("innerText");
    console.log("Checking to make sure cart is still 20.");
    expect(cartcount["_remoteObject"].value).toBe("20");
  ...
  it("Checking the localStorage to make sure cart is correct", async () => {
    console.log("Check to make sure local storage has 20 items in array.");

    const locStorage = await page.evaluate(() =>
      window.localStorage.getItem("cart")
    );

    expect(locStorage).toBe(
  ...
  it("Checking number of items in cart on screen after removing from cart", async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      console.log("Items Left in Cart: " + cartcount["_remoteObject"].value);
      expect(cartcount["_remoteObject"].value).toBe(`${--totalItems}`);
    }
  ...
  it("Checking number of items in cart on screen after reload", async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let shadowroot = await prodItems[i].getProperty("shadowRoot");
      let shadowbutton = await shadowroot.$("button");
      console.log(`Checking 'Add to Cart' for item ${i + 1}`);
      let innerTextProp = await shadowbutton.getProperty("innerText");
    ...
    let cart = await page.$("#cart-count");
    let cartcount = await cart.getProperty("innerText");
    console.log("Checking to make sure cart is still 0.");
    expect(cartcount["_remoteObject"].value).toBe("0");
